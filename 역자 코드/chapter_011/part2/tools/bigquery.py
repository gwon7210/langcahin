...
class BigQueryClient():
    ...
    def _exec_query(self, query: str, limit: int = None) -> pd.DataFrame:
        ...
        """ SQL을 실행해서 Pandas DataFrame으로 반환합니다 """
    ...
    def exec_query_tool(self):
        ...
        이 도구를 사용하기 전에 get_table_info_tool로
        ...
        project_id, dataset_id, table_id를 반드시 지정해 주세요.


        사용 중인 BigQuery는 다음과 같습니다:
        - project_id: {self.dataset_project_id}
        - dataset_id: {self.dataset_id}
        - table_id: {self.table_names_str}


        SQL은 가독성을 고려해서 작성해 주세요 (예: 줄바꿈 등).

        최빈값을 구할 때는 "Mod" 함수를 사용해 주세요.


        샘플 이외의 결과는 Code Interpreter에 의해 CSV 파일로 저장됩니다.

        Python을 실행해서 접근해 주세요.
        """
